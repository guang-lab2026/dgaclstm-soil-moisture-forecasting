...
class DGCLSTMV21(Module):
    def __init__(self, input_size, hidden_size, output_chans, filter_size, num_layers, *args):
        super().__init__()
    ...
    def forward(self, inputs, targets):
        ...
        for t in range(in_len + out_len):
            ...
            # c[0], h[0], m, h_DH = self.lstm[0](diff_h, input_next, c[0], h[0], m, h_DH, G_info)
            #
            ...
            for i in range(self.n_layers):
                c[i], h[i], m, h_DH = self.lstm[i](diff_h, input_next, c[i], h[i], m, h_DH, G_info)
                diff_h = h[i] - h_last[i]
                input_next = h[i]
...
class PredRNNDIFFGEO(Module):
    ...
    def forward(self, inputs, targets):
        ...
        for t in range(in_len + out_len):
            ...
            # c[0], h[0], m, h_DH[0] = self.lstm[0](diff_h, input_next, c[0], h[0], m, h_DH[0], G_info)
            # diff_h = h[0] - h_last[0]
            #
            
            # for i in range(1, self.n_layers):
            #     c[i], h[i], m, h_DH[i] = self.lstm[i](diff_h, h[i - 1], c[i], h[i], m, h_DH[i], G_info)
            #     diff_h = h[i] - h_last[i]

            for i in range(self.n_layers):
                c[i], h[i], m, h_DH[i] = self.lstm[i](diff_h, input_next, c[i], h[i], m, h_DH[i], G_info)
                diff_h = h[i] - h_last[i]
                input_next = h[i]

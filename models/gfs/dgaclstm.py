...
class DGCLSTMDecoder(nn.Module):
    ...
    def __init__(self, input_channels: int, hidden_channels: int, output_channels: int,
                 kernel_size: int, num_layers: int = 3, forecast_chans: int = 0):
        super().__init__()
        self.num_layers = num_layers
        self.hidden_channels = hidden_channels
        ...
        if forecast_chans > 0:
            self.forecast_embed = nn.Conv2d(forecast_chans, hidden_channels, 1)
            actual_input_channels = input_channels + hidden_channels
        else:
            raise ValueError("")
            # self.forecast_embed = None
        ...
        self.input_proj = nn.Conv2d(actual_input_channels, hidden_channels, 1)

        
        self.output_conv = nn.Conv2d(hidden_channels, output_channels, 1)

    def forward(self, x: torch.Tensor, states: Dict,
                forecast: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Dict]:
        ""
        h = states['h']
        c = states['c']
        m = states['m']
        ...
        forecast_feat = self.forecast_embed(forecast)
        x = torch.cat([x, forecast_feat], dim=1)
...
class DGACLSTM(nn.Module):
    ...
    def __init__(self, input_channels: int, hidden_channels: int, output_channels: int,
                 kernel_size: int = 3, num_layers: int = 3, forecast_chans: int = 0, *args):  
        super().__init__()
        ...
        self.decoder = DGCLSTMDecoder(hidden_channels * 2, hidden_channels,
                                      output_channels, kernel_size, num_layers, forecast_chans)
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor = None,
                other_forecast: Optional[torch.Tensor] = None  
        ...
        for t in range(T_out):
            ...
            forecast_t = other_forecast[:, t] if other_forecast is not None else None
            output, decoder_states = self.decoder(decoder_input, decoder_states, forecast_t)
...
class EncoderDecoderNoAttention(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor = None,
        ...
        for t in range(T_out):
            ...
            forecast_t = other_forecast[:, t] if other_forecast is not None else None
            output, decoder_states = self.decoder(last_output, decoder_states, forecast_t)
